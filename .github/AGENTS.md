...
対象ファイル: src/presentation/kanbanView.ts, src/sync/syncEngine.ts
受け入れ条件:
- フィルター入力欄が追加される
- Markdown 可逆性を損なわない
- テスト coverage ≥70%
...
目的: ステータス表示領域に sync state を表示する
受け入れ条件:
...
新しい sync ステータス表示機能に対して検証計画を作りたい。

要件ID: FR-CMD-04, FR-SYNC-04, NFR-ACC-01
対象範囲: src/main.ts, src/test/ui.test.ts, src/test/sync.test.ts
期待:
...
sync ステータス表示変更に合わせて設計書と検証結果を更新したい。
...
sync ステータス表示の更新処理を調査したい。

作業目的: 実装前の調査と更新方針整理
変更対象ファイル: src/main.ts, src/sync/syncEngine.ts
制約条件:
- Markdown as Truth を維持する
- Notice ベースの通知方針を壊さない
...
- sync 層: ✓影響なし
- presentation 層: ✓ UI 側で表示対応（別PR予定）
- plugin 層: ✓ 影響なし
...
### Architecture
- **Layer Isolation**: Plugin → Presentation/Sync → Parser → Domain（逆依存なし）
