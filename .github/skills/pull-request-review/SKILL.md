...
Weekly Kanban VS Code Extension の PR 差分を、設計仕様と検証仕様に照らしてレビューする。
...
1. 差分を層ごとに分類する（extension/domain/parser/sync/webview/tests/docs）。
...
1. 依存方向（Extension -> Sync/Webview -> Parser -> Domain）が維持されている。
...
1. 変更に対応するテストが存在する。
2. 既存テストを削減していない。
3. エラーハンドリング方針（StatusMessage / Result ベース）を壊していない。
